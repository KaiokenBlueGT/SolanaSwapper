    ...
    private void RenderMenuBar()
    {
        ...
        if (ImGui.BeginMenu("File"))
        {
            ...
            if (ImGui.BeginMenu("Export"))
            {
                ...
                {
                    if (AssetLoader != null && EntityManager.Singleton.GetAllEntities().Count > 0)
                    {
                        AddFrame(new LevelExportFrame());
                    }
                    else
                    {
                        // Show error - no level loaded
                        LunaLog.LogError("No level loaded for export!");
                    }
                }
                ImGui.EndMenu();
            }
