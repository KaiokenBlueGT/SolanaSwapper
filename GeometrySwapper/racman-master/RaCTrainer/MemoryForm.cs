...
{ 
    public partial class MemoryForm : Form
    {
        ...
        public void PopulateMobysComboBoxRac1()
        {
            // Clear combo box
            selectedMobyComboBox.Items.Clear();

            var pid = func.api.getCurrentPID();
            uint instance = BitConverter.ToUInt32(func.api.ReadMemory(pid, rac1.addr.mobyInstances, 4).Reverse().ToArray(), 0);
            uint end = BitConverter.ToUInt32(func.api.ReadMemory(pid, rac1.addr.mobyInstancesEnd, 4).Reverse().ToArray(), 0);

            while (instance < end) {
                ushort oClass = BitConverter.ToUInt16(func.api.ReadMemory(pid, instance + 0xa6, 0x2).Reverse().ToArray(), 0);
                byte state = func.api.ReadMemory(pid, instance + 0x20, 0x1)[0];
        ...
        public void PopulateMobyInspectorRac1(int index)
        {   
            var pid = func.api.getCurrentPID();
            uint instance = BitConverter.ToUInt32(func.api.ReadMemory(pid, rac1.addr.mobyInstances, 4).Reverse().ToArray(), 0);

            byte[] memory = func.api.ReadMemory(pid, instance + (0x100 * (uint)index), 0x100);
            rac1.Moby moby = rac1.Moby.ByteArrayToMoby(memory);

            var type = typeof(rac1.Moby);
            var fields = type.GetFields();
            ...
            for (int i = 0; i < fields.Length; i++)
            {
                ...
                if (field.FieldType == typeof(rac1.Vec4))
                {
                    rac1.Vec4 vec = (rac1.Vec4)field.GetValue(moby);
                    item.SubItems[2].Text = $"x: {vec.x}, y: {vec.y}, z: {vec.z}, w: {vec.w}";
                }
                else if(field.FieldType == typeof(rac1.GamePtr))
                {
                    item.SubItems[2].Text = $"0x{((rac1.GamePtr)field.GetValue(moby)).addr.ToString("X")}";
                }
                else
                {
                    item.SubItems[2].Text = field.GetValue(moby).ToString();
        ...
        {
            if (AttachPS3Form.game == "NPEA00386") // RAC2
            {
            ...
            else if (AttachPS3Form.game == "NPEA00385") // RAC1
            {
                WebMAN wmm = new WebMAN(func.api.GetIP());
                wmm.PauseRSX();
        ...
        private void exportAllMobyDataButton_Click(object sender, EventArgs e)
        {
            SaveFileDialog saveFileDialog = new SaveFileDialog
            {
                Filter = "Text File|*.txt",
                Title = "Save Moby Data",
                FileName = "moby_data.txt"
            };
            ...
            try
            {
                ...
                if (AttachPS3Form.game == "NPEA00386") // RAC2
                {
                    mobyData = ExportAllMobyDataRac2();
                }
                else if (AttachPS3Form.game == "NPEA00385") // RAC1
                {
                    mobyData = ExportAllMobyDataRac1();
                }
                else
                {
                    MessageBox.Show("Game is not supported.");
                ...
                File.WriteAllText(saveFileDialog.FileName, mobyData);
                MessageBox.Show("Moby data exported successfully!", "Export Complete", MessageBoxButtons.OK, MessageBoxIcon.Information);
            }
            catch (Exception ex)
            {
        ...
        private string ExportAllMobyDataRac1()
        {
            var pid = func.api.getCurrentPID();
            uint instance = BitConverter.ToUInt32(func.api.ReadMemory(pid, rac1.addr.mobyInstances, 4).Reverse().ToArray(), 0);
            uint end = BitConverter.ToUInt32(func.api.ReadMemory(pid, rac1.addr.mobyInstancesEnd, 4).Reverse().ToArray(), 0);
            ...
            while (instance < end)
            {
                byte[] memory = func.api.ReadMemory(pid, instance, 0x100);
                rac1.Moby moby = rac1.Moby.ByteArrayToMoby(memory);

                sb.AppendLine($"Moby at 0x{instance:X}");
                sb.AppendLine("--------------------");
                ...
                foreach (var field in fields)
                {
                    ...
                    if (field.FieldType == typeof(rac1.GamePtr))
                    {
                        valueString = $"0x{((rac1.GamePtr)value).addr:X}";
                    }
                    else if (field.FieldType == typeof(rac1.Vec4))
                    {
                        rac1.Vec4 vec = (rac1.Vec4)value;
                        valueString = $"x: {vec.x}, y: {vec.y}, z: {vec.z}, w: {vec.w}";
                    }
                    else
                    {
        ...
        private string ExportAllMobyDataRac2()
        {
            var pid = func.api.getCurrentPID();
            uint instance = BitConverter.ToUInt32(func.api.ReadMemory(pid, rac2.addr.mobyInstances, 4).Reverse().ToArray(), 0);
            uint end = BitConverter.ToUInt32(func.api.ReadMemory(pid, rac2.addr.mobyInstancesEnd, 4).Reverse().ToArray(), 0);
            ...
            while (instance < end)
            {
                ...
                foreach (var field in fields)
                {
                    ...
                    if (field.FieldType == typeof(rac2.GamePtr))
                    {
                        valueString = $"0x{((rac2.GamePtr)value).addr:X}";
                    }
                    else if (field.FieldType == typeof(rac2.Vec4))
                    {
                        rac2.Vec4 vec = (rac2.Vec4)value;
                        valueString = $"x: {vec.x}, y: {vec.y}, z: {vec.z}, w: {vec.w}";
                    }
                    else
                    {
        ...
        private void selectedMobyComboBox_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            else if (AttachPS3Form.game == "NPEA00385") // RAC1
            {
                timer.Elapsed += (s, evt) =>
                {
                    ...
                    this.Invoke(new Action(() =>
                    {
                        PopulateMobyInspectorRac1(((ComboBox)sender).SelectedIndex);
                    }));
                };
                timer.Start(); // Start the timer
